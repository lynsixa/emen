...
<div class="container py-5">
    ...
    <form action="{{ route('admin.usuario.update', $usuario->idUsuario) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="form-group mb-3">
            ...
            <input type="date" class="form-control" name="FechaDeNacimiento" value="{{ $usuario->FechaDeNacimiento }}" required>
        </div>
        ...
        <div class="form-group mb-3">
            ...
            <select name="Tipo_de_documento_idTipodedocumento" class="form-control" required>
                @foreach($tipos as $tipo)
                    <option value="{{ $tipo->idTipodedocumento }}" 
                        {{ $usuario->Tipo_de_documento_idTipodedocumento == $tipo->idTipodedocumento ? 'selected' : '' }}>
                        {{ $tipo->Descripcion }}
                    </option>
                @endforeach
            </select>
            ...
            <select name="Roles_idRoles" class="form-control" required>
                @foreach($roles as $rol)
                    <option value="{{ $rol->idRoles }}" 
                        {{ $usuario->Roles_idRoles == $rol->idRoles ? 'selected' : '' }}>
                        {{ $rol->Descripcion }}
                    </option>
                @endforeach
            </select>
        ...
        <!-- Este campo de contraseña ha sido eliminado, ya que no debe ser editable -->
